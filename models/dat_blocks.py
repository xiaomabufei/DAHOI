...
class LocalAttention(nn.Module):
    ...
    def forward(self, x, H, W, mask=None):
        ...
        pad_l = pad_t = 0
        pad_r = (self.window_size[1] - _W % self.window_size[1]) % self.window_size[1]
        pad_b = (self.window_size[0] - _H % self.window_size[0]) % self.window_size[0]
        ...
        if mask is not None:
            ...
            # mask : nW ww ww
            nW, ww, _ = mask.size()
            attn = einops.rearrange(attn, '(b n) h w1 w2 -> b n h w1 w2', n=nW, h=self.heads, w1=ww, w2=ww) + mask.reshape(1, nW, 1, ww, ww).to(attn.device)
        ...
        if pad_r > 0 or pad_b > 0:
            x = x[:, :, :_H, :_W].contiguous()
        return x, None, None, x.size(2), x.size(3)

class ShiftWindowAttention(LocalAttention):

    def __init__(self, dim, heads, window_size, attn_drop, proj_drop, shift_size):
    ...
    def forward(self, x, H, W):
        ...
        for h in h_slices:
            for w in w_slices:
                img_mask[h, w] = cnt
                cnt += 1
        mask_windows = einops.rearrange(img_mask, '(r1 h1) (r2 w1) -> (r1 r2) (h1 w1)', h1=self.window_size[0],w1=self.window_size[1])  # nW, window_size, window_size, 1
        mask_windows = mask_windows.view(-1, self.window_size[0] * self.window_size[1])
        attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
        attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))
        shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(2, 3))
        sw_x, _, _, _, _ = super().forward(shifted_x, x.size(2), x.size(3), attn_mask)
...
class DAttentionBaseline(nn.Module):
    ...
    def __init__(
        self, stage, n_heads, n_head_channels, n_groups,
        ...
        self.q_h, self.q_w = None, None
        self.kv_h, self.kv_w = None, None
        ...
        self.conv_offset = nn.Sequential(
            GIM((kk, kk), self.n_group_channels, 2),
    ...
    def forward(self, x, Wh, Ww):
        ...
        # self.Global = Global_Interaction((self.q_h, self.q_w), (self.kk, self.kk), self.n_group_channels, 8)
        # q_off = self.Global(q_off)
        ...
        # return y, pos.reshape(B, self.n_groups, Hk, Wk, 2), reference.reshape(B, self.n_groups, Hk, Wk, 2), y.size(2), y.size(3)
        return y, pos.reshape(B, self.n_groups, Hk, Wk, 2), reference.reshape(B, self.n_groups, Hk, Wk, 2), y.size(2), y.size(3)
