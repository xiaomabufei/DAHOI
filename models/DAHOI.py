...
"""
DAHOI model and criterion classes.
"""
import torch
...
class DAHOI(nn.Module):
    """ This is the DAHOI module that performs human object interaction detection """
    def __init__(self, backbone, transformer, num_classes, num_verb_classes, num_queries, num_feature_levels,
        ...
        Parameters:
            ...
            num_verb_classes: number of interaction action classes
            num_queries: number of human-object pair queries, ie detection slot.
    ...
    def forward(self, samples: NestedTensor):
        ...
        features, pos, references = self.backbone(samples)
        anchor = references[3][1]
        anchor = [x[5] for x in anchor]
        anchor_all = [x.view(-1, 2) for x in anchor]
        dynamic_anchor = Radar_sampling(self.num_queries, self.N, anchor_all)
        srcs = []
        masks = []
        ...
        hs, init_reference, inter_references = self.transformer(srcs, masks, pos, dynamic_anchor, query_embeds)
